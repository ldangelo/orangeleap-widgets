package com.orangeleap.service.impl;
...
import com.orangeleap.domain.ACHPaymentInfo;
...
import com.orangeleap.service.DonateNow;
...
public class DonateNowImpl implements DonateNow {
	...
	public PaymentStatus donate(Donation d) throws DatatypeConfigurationException {
		...
		oleap = client.getOrangeLeap("http://localhost:8080/orangeleap/services/1.0/orangeleap.wsdl", "nolan@company1", "ryan");
	...
	private Constituent findConstituent(Donation d) {
		...
		if (constituents.size() > 0) return constituents.get(0);
		else return null;
	}
	...
	private Constituent createConstituent(Donation d) {
		...
		SaveOrUpdateConstituentResponse response = oleap.saveOrUpdateConstituent(request);
		return response.getConstituent();
	}
	
	private Gift addGift(Constituent c, Donation d) throws DatatypeConfigurationException {
		Gift g = new Gift();
		CustomFieldMap fieldMap = new CustomFieldMap();
		...
		g.setDonationDate(DatatypeFactory.newInstance().newXMLGregorianCalendar(cal));
		...
		if (d.getPaymentInfo() instanceof CreditCardPaymentInfo) {
			g.setPaymentType("Credit Card");
		
		
			PaymentSource source = new PaymentSource();
			source.setConstituentId(c.getAccountNumber());
			source.setCreditCardHolderName(((CreditCardPaymentInfo) d.getPaymentInfo()).getName());		
			source.setCreditCardNumber(((CreditCardPaymentInfo) d.getPaymentInfo()).getCcNumber());
			source.setCreditCardExpirationMonth(((CreditCardPaymentInfo) d.getPaymentInfo()).getCcExpMonth());
			source.setCreditCardExpirationYear(((CreditCardPaymentInfo) d.getPaymentInfo()).getCcExpYear());
			source.setPaymentType(PaymentType.CREDIT_CARD);

			g.setPaymentSource(source);
		} else if (d.getPaymentInfo() instanceof ACHPaymentInfo) {
			g.setPaymentType("ACH");
			PaymentSource source = new PaymentSource();
			source.setConstituentId(c.getAccountNumber());
			
			source.setAchAccountNumber(((ACHPaymentInfo) d.getPaymentInfo()).getACHAccountNumber());
			source.setAchRoutingNumber(((ACHPaymentInfo) d.getPaymentInfo()).getACHRoutingNumber());
			source.setAchHolderName(((ACHPaymentInfo) d.getPaymentInfo()).getACHName());			
	...
	private PaymentStatus createPaymentStatus(Gift g) {
		...
		status.setAutorizationCode(g.getAuthCode());
		status.setMessage(g.getPaymentMessage());
		if (g.getAuthCode() != null && g.getAuthCode() != "") status.setProcessed(true);
		return status;
	}
}
